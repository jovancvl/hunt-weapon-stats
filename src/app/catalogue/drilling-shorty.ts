import { AmmoInfo } from "../model/ammo-info";
import { AmmoStats } from "../model/ammo-stats";
import { DamageBreakpoint } from "../model/damage"
import { ActionType, Weapon } from "../model/weapon";

const BASE_DAMAGE = 118
const OPTIMAL_RANGE = 30
const DROP_RANGE = 120
const SPREAD = 62.5
const VERTICAL_RECOIL = 16
const MUZZLE_VELOCITY = 424
const AMMO_RESERVE = 20

const NAME = "Drilling Shorty"
const COST = 330
const SIZE = 2
const ACTION = ActionType.SEMI_OR_FULL_AUTO
const SWAY = 100
const RATE_OF_FIRE = 20
const CYCLE_TIME = 0.6
const RELOAD_TIME = 4.8
const MAGAZINE = 2
const HAS_EXTRA_BULLET = false
const MELEE_DAMAGE = 13
const STAMINA_CONSUMPTION = 15
const HEAVY_MELEE_DAMAGE = 31
const HEAVY_STAMINA_CONSUMPTION = 25
const IMAGE = "https://huntshowdown.wiki.gg/images/Weapon_Drilling_Shorty.png"
const BASE_AMMO_DAMAGE_BREAKPOINTS: DamageBreakpoint[] = [
  {
    range: 0,
    damage: BASE_DAMAGE
  },
  {
    range: OPTIMAL_RANGE,
    damage: BASE_DAMAGE
  },
  {
    range: 80,
    damage: BASE_DAMAGE
  },
  {
    range: 100,
    damage: BASE_DAMAGE
  },
]
/**
 * Includes all available ammo types, including the base ammo, which is always first.
 */
const AMMO = [
  new AmmoStats({
    info: AmmoInfo.MEDIUM,
    scarce: false,

    dropRange: DROP_RANGE,
    spread: SPREAD,
    verticalRecoil: VERTICAL_RECOIL,
    muzzleVelocity: MUZZLE_VELOCITY,
    ammoReserve: AMMO_RESERVE,
    damageBreakpoints: BASE_AMMO_DAMAGE_BREAKPOINTS
  }),
  new AmmoStats({
    info: AmmoInfo.MEDIUM_DUMDUM,
    scarce: false,

    dropRange: 105,
    spread: SPREAD,
    verticalRecoil: VERTICAL_RECOIL,
    muzzleVelocity: 340,
    ammoReserve: AMMO_RESERVE,
    damageBreakpoints: BASE_AMMO_DAMAGE_BREAKPOINTS
  }),
  new AmmoStats({
    info: AmmoInfo.MEDIUM_FMJ,
    scarce: false,

    dropRange: 105,
    spread: SPREAD,
    verticalRecoil: 20,
    muzzleVelocity: 340,
    ammoReserve: AMMO_RESERVE,
    damageBreakpoints: [
      {
        range: 0,
        damage: BASE_DAMAGE
      },
      {
        range: 40,
        damage: BASE_DAMAGE
      },
      {
        range: 90,
        damage: BASE_DAMAGE
      },
      {
        range: 100,
        damage: BASE_DAMAGE 
      },
    ]
  }),
  new AmmoStats({
    info: AmmoInfo.MEDIUM_HIGH_VELOCITY,
    scarce: false,

    dropRange: 135,
    spread: SPREAD,
    verticalRecoil: 22,
    muzzleVelocity: 524,
    ammoReserve: 13,
    damageBreakpoints: [
      {
        range: 0,
        damage: 113
      },
      {
        range: OPTIMAL_RANGE,
        damage: 113
      },
      {
        range: 80,
        damage: BASE_DAMAGE
      },
      {
        range: 100,
        damage: BASE_DAMAGE
      },
    ]
  }),
]

export const DRILLING_SHORTY: Weapon = new Weapon({
  name: NAME,
  cost: COST,
  size: SIZE,
  action: ACTION,

  availableAmmo: AMMO,

  sway: SWAY,
  rateOfFire: RATE_OF_FIRE,
  cycleTime: CYCLE_TIME,
  reloadTime: RELOAD_TIME,
  magazine: MAGAZINE,
  hasExtraBullet: HAS_EXTRA_BULLET,

  meleeDamage: MELEE_DAMAGE,
  staminaConsumption: STAMINA_CONSUMPTION,
  heavyMeleeDamage: HEAVY_MELEE_DAMAGE,
  heavyStaminaConsumption: HEAVY_STAMINA_CONSUMPTION,

  image: IMAGE
})